...
class AuthLogoutRouteTest {
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'AuthLogoutRouteTest',
  ...
  buildBody(seed = 0) {
    ...
    return {
      ...{ actor: 'tester', reason: 'repro', feature: 'AuthLogoutRouteTest' },
      seed,
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('AuthLogoutRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error('AuthLogoutRouteTest: unexpected base route in payload');
    }

    if (payload.className !== 'AuthLogoutRouteTest') {
      throw new Error('AuthLogoutRouteTest: unexpected className in payload');
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'AuthLogoutRouteTest: expected all status codes to be 200',
      );
    }
