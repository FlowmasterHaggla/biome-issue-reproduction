...
class AdminAuditRouteTest {
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'AdminAuditRouteTest',
  ...
  buildBody(seed = 0) {
    ...
    return {
      ...{ actor: 'tester', reason: 'repro', feature: 'AdminAuditRouteTest' },
      seed,
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('AdminAuditRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error('AdminAuditRouteTest: unexpected base route in payload');
    }

    if (payload.className !== 'AdminAuditRouteTest') {
      throw new Error('AdminAuditRouteTest: unexpected className in payload');
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'AdminAuditRouteTest: expected all status codes to be 200',
      );
    }
