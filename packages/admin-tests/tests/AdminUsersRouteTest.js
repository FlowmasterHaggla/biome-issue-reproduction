...
class AdminUsersRouteTest {
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'AdminUsersRouteTest',
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('AdminUsersRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error('AdminUsersRouteTest: unexpected base route in payload');
    }

    if (payload.className !== 'AdminUsersRouteTest') {
      throw new Error('AdminUsersRouteTest: unexpected className in payload');
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'AdminUsersRouteTest: expected all status codes to be 200',
      );
    }
