...
class ContentDeleteRouteTest {
  constructor() {
    this.scope = 'content';
    this.baseRoute = '/content/delete';
    this.defaultMethod = 'DELETE';
    this.version = 'v1';
  }
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'ContentDeleteRouteTest',
  ...
  buildBody(seed = 0) {
    ...
    return {
      ...{
        actor: 'tester',
        reason: 'repro',
        feature: 'ContentDeleteRouteTest',
      },
      seed,
    ...
    app.route({
      ...
      handler: async (request) => {
        return {
          ...
          method: this.defaultMethod,
          scope: this.scope,
          className: 'ContentDeleteRouteTest',
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('ContentDeleteRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error(
        'ContentDeleteRouteTest: unexpected base route in payload',
      );
    }

    if (payload.className !== 'ContentDeleteRouteTest') {
      throw new Error(
        'ContentDeleteRouteTest: unexpected className in payload',
      );
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'ContentDeleteRouteTest: expected all status codes to be 200',
      );
    }
