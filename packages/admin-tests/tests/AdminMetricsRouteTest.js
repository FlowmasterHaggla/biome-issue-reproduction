...
class AdminMetricsRouteTest {
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'AdminMetricsRouteTest',
    ...
    app.route({
      ...
      handler: async (request) => {
        return {
          ...
          method: this.defaultMethod,
          scope: this.scope,
          className: 'AdminMetricsRouteTest',
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('AdminMetricsRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error(
        'AdminMetricsRouteTest: unexpected base route in payload',
      );
    }

    if (payload.className !== 'AdminMetricsRouteTest') {
      throw new Error('AdminMetricsRouteTest: unexpected className in payload');
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'AdminMetricsRouteTest: expected all status codes to be 200',
      );
    }
