...
class ContentUpdateRouteTest {
  constructor() {
    this.scope = 'content';
    this.baseRoute = '/content/update';
    this.defaultMethod = 'PUT';
    this.version = 'v1';
  }
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'ContentUpdateRouteTest',
  ...
  buildBody(seed = 0) {
    ...
    return {
      ...{
        actor: 'tester',
        reason: 'repro',
        feature: 'ContentUpdateRouteTest',
      },
      seed,
    ...
    app.route({
      ...
      handler: async (request) => {
        return {
          ...
          method: this.defaultMethod,
          scope: this.scope,
          className: 'ContentUpdateRouteTest',
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('ContentUpdateRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error(
        'ContentUpdateRouteTest: unexpected base route in payload',
      );
    }

    if (payload.className !== 'ContentUpdateRouteTest') {
      throw new Error(
        'ContentUpdateRouteTest: unexpected className in payload',
      );
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'ContentUpdateRouteTest: expected all status codes to be 200',
      );
    }
