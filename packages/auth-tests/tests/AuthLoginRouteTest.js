...
class AuthLoginRouteTest {
  ...
  buildHeaders(overrides = {}) {
    return {
      ...
      'x-repro-class': 'AuthLoginRouteTest',
  ...
  assertBaseResponse(payload) {
    if (!payload || payload.ok !== true) {
      throw new Error('AuthLoginRouteTest: expected ok=true payload');
    }

    if (payload.route !== this.baseRoute) {
      throw new Error('AuthLoginRouteTest: unexpected base route in payload');
    }

    if (payload.className !== 'AuthLoginRouteTest') {
      throw new Error('AuthLoginRouteTest: unexpected className in payload');
    }
  }
  ...
  async run() {
    ...
    if (
      ...
      throw new Error(
        'AuthLoginRouteTest: expected all status codes to be 200',
      );
    }
